...
def extract_preview(
    ...
    if engine == "none" or extract_swf_frames is None:
        logging.debug("Skipping preview for %s (engine=%s)", swf_path, engine)
        return
    if not swf_path.exists():
        logging.warning("SWF not found for preview: %s", swf_path)
        return
        ...
        try:
            if engine == "auto":
                ...
                if shutil.which("ffmpeg"):
                    chosen = "ffmpeg"
                    extract_swf_frames.ffmpeg_extract(str(swf_path), str(tmpdir_path))  # type: ignore[attr-defined]
                elif ffdec and ffdec.exists():
                    chosen = "jpexs"
                    extract_swf_frames.jpexs_extract(  # type: ignore[attr-defined]
                        str(swf_path),
                        str(tmpdir_path),
                        str(ffdec),
                        select=f"{target_frame}-{target_frame}",
                    )
                elif shutil.which("swfrender") and shutil.which("swfdump"):
                    chosen = "swfrender"
                    ...
                    return
                logging.debug("Preview engine for %s: %s", swf_path, chosen)
            elif engine == "ffmpeg":
                extract_swf_frames.ffmpeg_extract(str(swf_path), str(tmpdir_path))  # type: ignore[attr-defined]
            elif engine == "jpexs":
                if not ffdec:
                    logging.warning("--ffdec is required for the JPEXS engine; skipping %s", swf_path)
                    return
                extract_swf_frames.jpexs_extract(  # type: ignore[attr-defined]
                    str(swf_path),
                    str(tmpdir_path),
                ...
                logging.warning("Unknown engine '%s' for %s", engine, swf_path)
                return
        except Exception as exc:  # pragma: no cover - passthrough tool failures
            logging.warning("Frame extraction failed for %s: %s", swf_path, exc)
            return

        desired = tmpdir_path / f"frame_{target_frame:05d}.png"
        if not desired.exists():
            candidates = sorted(tmpdir_path.glob("frame_*.png"))
            if not candidates:
                logging.warning("No frames generated for %s", swf_path)
                return
            desired = candidates[0]
        ensure_directory(dest_png.parent)
        shutil.copy2(desired, dest_png)
...
def copy_assets(
    ...
    for record in records:
        ...
        dest_png = (img_root / str(record.id) / Path(record.image_rel_path).name)
        extract_preview(dest_swf, dest_png, engine, record.frame, ffdec)
...
def main() -> None:
    ...
    if args.ffdec is None:
        ...
        args.ffdec = args.ffdec.expanduser()
        if not args.ffdec.exists():
            logging.warning("Provided --ffdec path does not exist: %s", args.ffdec)
    ...
    if not args.skip_media:
        ...
        copy_assets(repo_root, output_root, records, args.engine, args.ffdec)
    else:
