...
import fs from 'fs';
import { getHash, LabelFile } from "../labels/labels";
 ...
 * @param getMediaPath A function that takes the file name (no extension) and returns the path within the media folder to that file
 * @param autogeneratedHeader The title for the header in the autogenerated.txt label file
 */
export async function generateCrumbFiles<Modifications>(
  ...
  // clearing all files in the folder first
  const crumbsFolder = path.join(__dirname, '..', 'media', mediaPath);
  const filesInFolder = fs.readdirSync(crumbsFolder);
  filesInFolder.forEach((file) => {
    if (file.endsWith('.swf')) {
      fs.unlinkSync(path.join(crumbsFolder, file));
    }
  });
  ...
  const partyCrumbs = new Map<string, string>();
  ...
  seasonalCrumbs.set(PLACEHOLDER, nonPartyCrumbs);
  ...
  for (const pair of days) {
    console.log(`Progress: ${progress}/${days.length}`);
    progress++;
    const [date, crumb] = pair;
    // doing it in batches to not take all the memory
    if (promises.length >= 10) {
      await Promise.all(promises);
      promises = [];
    }
    promises.push(createSeasonalCrumb(crumb, date));
  }

  
  // waiting all SWFs to be finished
  await Promise.all(promises);

  // update labels for autogenerated.txt
  const labelFile = await LabelFile.fromFile(path.join(__dirname, '..', 'labels/groups/autogenerated.txt'));
  // first delete any previous text here, and for that it must have been manually added earlier
  const lines = labelFile.lines;
  let headerIndex = -1;
  let autogeneratedSectionEndIndex = -1;
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line === `- ${autogeneratedHeader}`) {
      headerIndex = i;
      for (let j = i + 1; j < lines.length; j++) {
        if (typeof lines[j] === 'string') {
          autogeneratedSectionEndIndex = j;
          break;
        }
      }
      break;
  ...
  if (headerIndex === -1 || autogeneratedSectionEndIndex === -1) {
    console.log('Could not add into autogenerate.txt');
  } else {
    // - 1 so that it doesn't actually delete the blank space
    const deleteLines = autogeneratedSectionEndIndex - headerIndex - 1;
    for (let i = 0; i < deleteLines; i++) {
      labelFile.deleteLine(headerIndex + 1);
    }
    days.forEach((pair) => {
      const file = pair[0];
      const filePath = path.join(mediaPath, file + '.swf')
      const hash = getHash(filePath);
      labelFile.insertLineAfterIndex(headerIndex, [filePath, hash, '']);
    });
    labelFile.write();
  }
}