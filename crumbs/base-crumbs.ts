...
export async function generateCrumbFiles<Modifications>(
  ...
  const createSeasonalCrumbWrapper = (content: string, fileName: string) => {
    filesAdded.push(fileName);
    filePromises.push(createSeasonalCrumb(content, fileName));
  }

  createSeasonalCrumbWrapper(nonPartyCrumbs, PLACEHOLDER);
  ...
  for (const update of crumbsUpdates) {
    if (update.type === 'seasonal') {
      ...
      createSeasonalCrumbWrapper(inPartyCrumbs, update.start);
    }
  }

  seasonalCrumbs.forEach((crumbContent, fileName) => {
    createSeasonalCrumbWrapper(crumbContent, fileName);
  })
  ...
  if (headerIndex === -1 || autogeneratedSectionEndIndex === -1) {
    ...
    filesAdded.forEach((file) => {
      const mediaPath = getMediaPath(file);
      const hash = getHash(mediaPath);
      labelFile.insertLineAfterIndex(headerIndex, [mediaPath, hash, '']);
    });
    labelFile.write();
  }
}